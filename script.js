...
const adminUsers = {
    ...
    "lwklowkwkwk@gmail.com": true,
    "apalahdawg@gmail.com": true
};

const specialUsers = {
    "4AMUSgR0pLSndwsrTUzazaXXybl2": "MEDAN"
};
...
window.saveDisplayName = function() {
    ...
    if (newName) {
        ...
        // Update all previous messages with the new name
        if (currentUser) {
...
window.saveAvatar = async function() {
    ...
    try {
        ...
        // Update all previous messages with the new avatar
        if (currentUser) {
            const userId = currentUser.uid;
            messagesRef.orderByChild("userId").equalTo(userId).once("value", (snapshot) => {
                const updates = {};
...
window.replyToMessage = function(messageId, event) {
    ...
    messageRef.once("value", (snapshot) => {
        ...
        if (messageData) {
            ...
            document.getElementById("replyText").textContent = messageData.text.length > 30 ? 
...
window.sendMessage = async function() {
    ...
    const messageData = {
        userId: user.uid,
        user: getDisplayName() || user.displayName,
        email: user.email,
        photoURL: getAvatarUrl() || user.photoURL,
        text: text,
        timestamp: Date.now()
    };
...
auth.onAuthStateChanged((user) => {
    ...
    if (user) {
        ...
        messagesRef.orderByChild("timestamp").limitToLast(100).on("child_added", (snapshot) => {
            ...
            const isKoruptor = ["lwklowkwkwk@gmail.com", "dnzobro@gmail.com", "commentfotocik@gmail.com", "email4@gmail.com"].includes(messageData.email);
            
            let fileContent = "";
            if (messageData.fileUrl) {
                if (messageData.fileType && messageData.fileType.startsWith("image")) {
                    fileContent = `<img src="${messageData.fileUrl}" alt="File upload" onerror="this.style.display='none'" style="cursor:pointer">`;
                } else if (messageData.fileType && messageData.fileType.startsWith("video")) {
                    fileContent = `<video src="${messageData.fileUrl}" controls style="max-width:100%;max-height:200px;border-radius:var(--border-radius-sm);margin-top:8px"></video>`;
                } else {
                    fileContent = `<a href="${messageData.fileUrl}" target="_blank" rel="noopener"><i class="fas fa-download"></i> Download File: ${messageData.fileName || "File"}</a>`;
                }
            }
            
            messageElement.innerHTML = `
                <img class="message-avatar" src="${escapeHtml(messageData.photoURL || "https://i.ibb.co/7W0Q6vz/default-avatar.png")}" alt="${escapeHtml(messageData.user)}" onerror="this.src='https://i.ibb.co/7W0Q6vz/default-avatar.png'">
                <div class="message-content">
                    ...
                    ${messageData.replyTo && !messageData.deleted ? `
                        <div style="font-size:0.8rem;color:var(--text-muted);background:rgba(255,255,255,.05);padding:6px;border-radius:8px;margin-bottom:6px">
                            <b>${escapeHtml(messageData.replyTo.user)}</b>: ${escapeHtml(messageData.replyTo.text)}
                        </div>
                    ` : ''}
                    ${messageData.deleted ? "<em>Pesan dihapus</em>" : escapedText}
                    ${fileContent}
                    <div class="timestamp-container">
                        ...
                        ` : ''}
                        ${(messageData.userId === user.uid || isAdmin(user.email)) && !messageData.deleted ? `
                            <button class="delete-btn" onclick="deleteMessage('${messageId}')" title="Hapus Pesan">
